...
elContent  = document.getElementById('readability-content') ;
rdbContent = elContent.innerHTML ;

rdbImages  = elContent.getElementsByTagName( 'img' ) ;
rdbLinks   = elContent.getElementsByTagName( 'a' ) ;
...
for( var i = 0; i < rdbImages.length; i++ ){
    elImgD = document.createElement( 'input' ) ;
    elImgD.name = 'imgdat[]' ;
    elImgD.value = rdbImages[i].src ;

    elFrm.appendChild( elImgD ) ;
}

// add link manifest to form
for( var i = 0; i < rdbLinks.length; i++ ){
    elLnkD = document.createElement( 'input' ) ;
    elLnkD.name = 'lnkdat[]' ;
    elLnkD.value = rdbLinks[i].href ;

    // TODO: check presence of href value here
